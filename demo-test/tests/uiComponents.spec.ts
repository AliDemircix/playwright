...
test("date pickers",async({page})=>{
  ...
  while(!calendarMonthAndYear?.includes(expectMonthAndYear)){
    await page.locator('in-calendar-pageable-navigation [data-name="chevron-right"]').click()
...
test("slider",async ({page})=>{
  ...
  const tempGauge = page.locator('[tabtitle="Temperature"] ngx-temparature-dragger circle')
...
const box = await tempBox.boundingBox() //always start top left coordinate
//@ts-ignore
